...
def start(update, context):
    buttons = ButtonMaker()
    buttons.buildbutton("ՏհɑƘíժɑ | Updates", "https://t.me/ShakidaUpdates")
...
def ping(update, context):
    uTime = get_readable_time(time() - botStartTime)
    start_time = int(round(time() * 1000))
    reply = sendMessage("Starting Ping", context.bot, update.message)
    end_time = int(round(time() * 1000))
    editMessage(f'<b>PONG!: </b>{end_time - start_time} ms\n<b>Uptime: </b>{uTime}', reply)
...
help = telegraph.create_page(
        title='ՏհɑƘíժɑ | Mirror-Leech-Bot Help',
